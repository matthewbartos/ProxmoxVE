...
function update_container() {
  ...
  if [[ "$os" == "ubuntu" || "$os" == "debian" ]]; then
    ...
    if pct exec "$container" -- [ -e /usr/bin/update ]; then
      if pct exec "$container" -- grep -q "matthewbartos/ProxmoxVE" /usr/bin/update; then
        ...
        pct exec "$container" -- bash -c "sed -i 's/tteck\\/Proxmox/matthewbartos\\/ProxmoxVE/g' /usr/bin/update"
...
header_info
echo -e "${GN}The process is complete. The repositories have been switched to matthewbartos/ProxmoxVE.${CL}\n"
