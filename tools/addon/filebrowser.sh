...
DB_PATH="/usr/local/matthewbartos/filebrowser.db"
DEFAULT_PORT=8080
...
ExecStartPre=/bin/touch /usr/local/matthewbartos/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/matthewbartos/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/matthewbartos/filebrowser.db -p ${PORT}
Restart=always
